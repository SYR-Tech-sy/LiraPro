...
function parseDevice(ua: string): {
  ...
  if (isTablet)
    return {
      type: "جهاز لوحي",
      name: detectBrowser(ua) + " — Tablet",
      icon: "tablet",
    };
  if (isPhone)
    return {
      type: "هاتف",
      name: detectBrowser(ua) + " — Mobile",
      icon: "phone",
    };
  return {
    type: "حاسوب",
    name: detectBrowser(ua) + " — Desktop",
    icon: "desktop",
  };
}
...
interface StoredSession {
  ...
  deviceName: string;
  deviceType: string;
  deviceIcon: "phone" | "tablet" | "desktop";
  os?: string;
  browser?: string;
  startedAt: string;
  lastSeenAt?: string;
...
function getSessions(userId: string): StoredSession[] {
  try {
    return JSON.parse(
      localStorage.getItem(SESSIONS_KEY_PREFIX + userId) ?? "[]",
    );
  } catch {
    return [];
  }
...
function ensureCurrentSession(userId: string): StoredSession[] {
  ...
  return existing.map((s) =>
    s.id === currentId ? { ...s, isCurrent: true } : s,
  );
}
...
export default function ProfilePage() {
  ...
  const [cardMode, setCardMode] = useState<"closed" | "view" | "edit">(
    "closed",
  );
  ...
  const [prevDeleteUserId, setPrevDeleteUserId] = useState<string | undefined>(
    undefined,
  );
  ...
  const lphId = useMemo(
    () => (userId ? getOrCreateLphId(userId) : ""),
    [userId],
  );

  const verifyKey = userId ? `syp-verify-status-${userId}` : "";
    ...
    queryFn: async () => {
      ...
      try {
        ...
        return data.map((s) => ({
          ...
          deviceIcon: (s.deviceType === "phone"
            ? "phone"
            : s.deviceType === "tablet"
              ? "tablet"
              : "desktop") as "phone" | "tablet" | "desktop",
          os: s.os || undefined,
  ...
  const removeSession = useCallback(
    async (sessionId: string) => {
      if (!userId) return;
      // Optimistic update
      queryClient.setQueryData<StoredSession[]>(["sessions", userId], (prev) =>
    ...
    try {
      ...
      updateProfile.mutate(
        { data: { profilePhoto: url } },
        {
          ...
          onError: () => {
            toast.error("فشل حفظ رابط الصورة");
          },
        },
      );
    } catch (err) {
      toast.error(
        err instanceof Error ? err.message : "حدث خطأ أثناء رفع الصورة",
      );
    } finally {
  ...
  const handleVerifyRequest = () => {
    ...
    const fullName =
      `${profile?.firstName || user?.user_metadata?.first_name || ""} ${profile?.lastName || user?.user_metadata?.last_name || ""}`.trim();
  ...
  const handleDeleteAccount = () => {
    ...
    const deleteKey = user?.id
      ? `syp-delete-request-${user.id}`
      : "syp-delete-request";
    ...
    try {
      ...
      const pData = profile as
        | { firstName?: string; lastName?: string }
        | undefined;
      const fullName =
        [pData?.firstName, pData?.lastName].filter(Boolean).join(" ") ||
  ...
  const {
    ...
    query: {
      enabled: !!isSignedIn,
      queryKey: ["profile", user?.id] as readonly unknown[],
    },
  });
  ...
  const forceComplete =
    !loadingProfile && isSignedIn && !!profile && !profile.profileCompleted;
      ...
      >
        ...
        <div>
          <h2 className="text-xl font-bold mb-2">
            تسجيل الدخول لعرض الملف الشخصي
          </h2>
  ...
  return (
    ...
    >
        ...
        <CardContent className="p-5 flex items-center gap-4 relative z-10">
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-0.5 flex-wrap mb-0.5">
              ...
              {verifyStatus === "approved" && <BlueBadge size={20} />}
            </div>
            <p
              className="text-primary-foreground/70 text-sm truncate"
              dir="ltr"
            >
            ...
            {lphId && (
              <p
                className="text-white/50 text-[10px] font-mono mt-0.5"
                dir="ltr"
              >
                {lphId}
              </p>
      ...
      {(verifyStatus === "none" || verifyStatus === "rejected") && (
        <Card
          className={`border shadow-sm ${verifyStatus === "rejected" ? "border-red-200 dark:border-red-900/40" : "border-border"}`}
        >
          <CardContent className="p-4 flex items-center gap-3">
            <div
              className={`w-10 h-10 rounded-xl flex items-center justify-center flex-shrink-0 ${verifyStatus === "rejected" ? "bg-red-50 dark:bg-red-900/20" : "bg-blue-50 dark:bg-blue-900/20"}`}
            >
            ...
            <div className="flex-1 min-w-0">
              <p
                className={`font-bold text-sm ${verifyStatus === "rejected" ? "text-red-700 dark:text-red-400" : "text-foreground"}`}
              >
                {verifyStatus === "rejected"
                  ? "تم رفض طلب التوثيق"
                  : "طلب توثيق الحساب"}
      ...
      {/* Wallet Button */}
      <button type="button" onClick={() => navigate("/app/wallet")} className="w-full">
          ...
          <CardContent className="p-4 flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-foreground/70 dark:text-white">
                  تتبع ممتلكاتك وأصولك
                </p>
              </div>
            </div>
      ...
      >
        ...
        <ChevronLeft
          className={`w-4 h-4 text-foreground/70 dark:text-white mr-auto transition-transform ${cardMode !== "closed" ? "rotate-90" : ""}`}
        />
      </button>
              ...
              <CardContent className="space-y-1">
                {[
                  ...
                  >
                    ...
                    <span
                      className={`text-sm font-medium text-right ${f.value ? "text-foreground" : "text-muted-foreground/50"}`}
                      dir={f.ltr ? "ltr" : "rtl"}
                    >
                      {f.value || "غير محدد"}
                    </span>
        ...
        >
          ...
          <div className="flex-1 text-right">
            ...
            <p className="text-[10px] text-muted-foreground">
              {sessions.length} جهاز نشط
            </p>
          </div>
          <ChevronLeft
            className={`w-4 h-4 text-muted-foreground transition-transform ${showSessions ? "rotate-90" : ""}`}
          />
        </button>
        <AnimatePresence>
          {showSessions && (
              ...
              <CardContent className="p-4 pt-0 flex flex-col gap-3">
                ...
                <div className="flex gap-2">
                  <button type="button"
                    onClick={() => {
                      if (user?.id && !showQr)
                        setQrToken(`lph-qrlogin:${user.id}:${Date.now()}`);
                      setShowQr((v) => !v);
                    }}
                  ...
                  {showQr && qrData && (
                    ...
                    >
                      ...
                      <p className="text-[10px] text-muted-foreground text-center">
                        امسح هذا الرمز من جهاز آخر لتسجيل الدخول
                      </p>
                      <p className="text-[9px] text-muted-foreground/60">
                        يتجدد الرمز عند كل زيارة
                      </p>
                    </motion.div>
                  )}
                </AnimatePresence>
                        ...
                        >
                          <div className="flex flex-col items-center gap-3">
                            ...
                            <p className="text-xs text-muted-foreground text-center leading-relaxed">
                              تم كشف رمز QR. هل تريد تسجيل هذا الجهاز للدخول
                              بهذا الحساب؟
                            </p>
                            <p
                              className="text-[10px] font-mono text-muted-foreground bg-secondary px-3 py-1.5 rounded-lg"
                              dir="ltr"
                            >
                          ...
                          <div className="flex gap-2">
                            ...
                            <Button
                              ...
                              onClick={() => {
                                if (user?.id) {
                                  const { name, icon, type } = parseDevice(
                                    navigator.userAgent,
                                  );
                ...
                <div className="flex flex-col gap-2">
                  {sessions.map((s) => {
                    ...
                    return (
                      <div
                        key={s.id}
                        className={`flex items-center gap-3 p-3 rounded-xl border transition-colors ${s.isCurrent ? "bg-primary/5 border-primary/20" : "bg-secondary/30 border-border"}`}
                      >
                        <div
                          className={`w-8 h-8 rounded-lg flex items-center justify-center flex-shrink-0 ${s.isCurrent ? "bg-primary/15" : "bg-secondary"}`}
                        >
                          <DevIcon
                            className={`w-4 h-4 ${s.isCurrent ? "text-primary" : "text-muted-foreground"}`}
                          />
                        </div>
                        <div className="flex-1 min-w-0">
                          ...
                          <p className="text-[9px] text-muted-foreground mt-0.5">
                            ...
                            {!s.isCurrent && s.lastSeenAt
                              ? `آخر نشاط: ${sessionTimeAgo(s.lastSeenAt)}`
                              : null}
                            {s.startedAt && (
                              <span className="opacity-60">
                                {s.isCurrent ? ` · ` : ""}
                                {`دخل: ${new Date(s.startedAt).toLocaleDateString("ar-SY")}`}
                              </span>
                            )}
                          </p>
                        </div>
                  ...
                  return (
                    <button type="button"
                      onClick={async () => {
                        ...
                        // Optimistic update
                        queryClient.setQueryData<StoredSession[]>(
                          ["sessions", user.id],
                          (prev) => (prev ?? []).filter((s) => s.isCurrent),
                        );
                        try {
                          const tok = await getToken();
                          if (tok) {
                            const deviceId =
                              localStorage.getItem("syp-device-id");
      ...
      <div className="flex flex-col gap-2">
        ...
        <div className="flex items-center gap-2">
          ...
          >
            <div
              className={`w-8 h-8 rounded-lg flex items-center justify-center ${deleteRequest ? "bg-secondary" : "bg-destructive/10"}`}
            >
                ...
                {deleteSuccess ? (
                  <div className="flex flex-col items-center gap-4 py-6">
                    ...
                    <p className="text-sm text-foreground/70 dark:text-white text-center leading-relaxed px-4">
                      سيتم حذف حسابك نهائياً بعد 30 يوماً. يمكنك الاستمرار في
                    ...
                    <div className="bg-destructive/5 border border-destructive/20 rounded-2xl p-4 flex flex-col gap-3">
                      <div className="flex items-start gap-2">
                        ...
                        <p className="text-xs font-bold text-destructive">
                          يرجى قراءة هذا بعناية قبل المتابعة:
                        </p>
                      </div>
                      <ul className="flex flex-col gap-2 text-xs text-foreground/70 dark:text-white leading-relaxed">
                        <li className="flex items-start gap-1.5">
                          <span className="text-primary mt-0.5">•</span>سيتم حذف
                          حسابك نهائياً بعد مرور 30 يوم من تقديم الطلب.
                        </li>
                        <li className="flex items-start gap-1.5">
                          <span className="text-primary mt-0.5">•</span>يمكنك
                          الاستمرار في استخدام التطبيق بشكل طبيعي طوال فترة الـ
                          30 يوم.
                        </li>
                        <li className="flex items-start gap-1.5">
                          <span className="text-primary mt-0.5">•</span>ستتلقى
                          إشعاراً قبل 3 أيام من موعد الحذف النهائي.
                        </li>
                        <li className="flex items-start gap-1.5">
                          <span className="text-destructive mt-0.5">•</span>لا
                          يمكن استعادة الحساب أو بياناتك بعد الحذف النهائي.
                        </li>
                      </ul>
                    </div>
        ...
        <AnimatePresence>
          {showDeleteDetails && deleteRequest && (
            <div className="fixed inset-0 z-[999] bg-black/50 flex items-center justify-center p-4">
              ...
              >
                <div className="flex items-center justify-between">
                  <button type="button" onClick={() => setShowDeleteDetails(false)}>
                    <X className="w-5 h-5 text-foreground" />
                  </button>
                ...
                <div className="bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-700/40 rounded-2xl p-4 flex flex-col gap-3">
                  ...
                  <div className="border-t border-amber-200 dark:border-amber-700/40 pt-3 flex flex-col gap-2">
                    <div className="flex justify-between items-center">
                      <span className="text-xs text-foreground/70 dark:text-white">
                        تاريخ تقديم الطلب
                      </span>
                      <span
                        className="text-xs font-bold text-foreground dark:text-white"
                        dir="ltr"
                      >
                        {new Date(deleteRequest.timestamp).toLocaleDateString(
                          "ar-SY",
                    ...
                    <div className="flex justify-between items-center">
                      ...
                      <span
                        ...
                        dir="ltr"
                      >
                        {new Date(deleteRequest.timestamp).toLocaleTimeString(
                          "ar-SY",
                ...
                <p className="text-xs text-foreground/70 dark:text-white text-center leading-relaxed">
                  يمكنك الاستمرار في استخدام التطبيق بشكل طبيعي حتى موعد الحذف
                  النهائي.
                </p>
                ...
                <div className="flex gap-2">
                  <button type="button"
                    onClick={() => {
                      ...
                      if (walletId) {
                        fetch(
                          `/api/admin/deletion-requests/cancel/${walletId}`,
                          { method: "DELETE" },
                        ).catch(() => {});
                        localStorage.removeItem(
                          `syp-delete-request-${walletId}`,
                        );
                      }
                      setDeleteRequest(null);
                ...
                <div className="p-5">
                  <div className="flex items-start gap-2 p-3 mb-4 rounded-xl bg-amber-50 dark:bg-amber-950/30 border border-amber-200 dark:border-amber-900/50">
                    ...
                    <p className="text-xs text-amber-800 dark:text-amber-300 leading-relaxed">
                      هذه البيانات ضرورية لتفعيل جميع ميزات المنصة. يمكنك
                      إكمالها لاحقاً من إعدادات ملفك الشخصي.
                    </p>
                  </div>
